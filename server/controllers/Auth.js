...
exports.sendOTP = async (req, res) => {
  try {
    ...
    // niche likhi line se phle PREMIDDLEWARE otp model ka chl jayega --> this.email and this.otp
    /**
...
exports.signUp = async (req, res) => {
  try {
    ...
    // basically sendotp vaale function ne otp create karke db me store kar di hogi .
    const dbOPT = await OTP.findOne({ email }).sort({ createdAt: -1 }).limit(1); // this will give most rececnt OTP
    console.log("db otp is :", dbOPT);
    if (!dbOPT) {
      return res.status(400).json({
        success: false,
  ...
  try {
    ...
    if (!exsistingUser) {
      return res.status(400).json({
        success: false,
        message:
          "User not registered with this email, Please enter valid email",
      });
    }
    ...
    //basically for create a token we need three things--1. Header. 2.Payload , 3.signature
    //here header contain algo info, payload contains user info, signature contain some fiels like expireIn time etc.
    ...
    //here we add token for response , in res we will send existinguser
    //existingUesr means jo database me save he . ab ham isko res me pass karenhe to iske saath token bhi add kar denge.
    exsistingUser.token = token;
    //password to hide or undefined kar denge.
